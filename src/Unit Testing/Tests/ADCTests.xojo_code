...
Inherits TestGroup
	...
	#tag Method, Flags = &h0
		Sub BCD61Test()
		  // Get the JSON contents of the opcode 61 file.
		  Var json As String = GetTestFileContents("61")
		  
		  Var allTests() As Variant = ParseJSON(json)
		  
		  // Gather up the tests where the decimal flag is 1 (BCD arithmetic).
		  Var bcdTests() As Dictionary
		  For Each test As Dictionary In allTests
		    Var initial As New CPUState(test.Value("initial"), test.Lookup("name", ""))
		    If initial.DecimalFlag Then
		      bcdTests.Add(test)
		    End If
		  Next test
		  
		  // Run each test.
		  For Each test As Dictionary In bcdTests
		    CPU = New MOS6502.CPU(New MOS6502.Memory)
		    CPU.Reset
	...
	#tag Method, Flags = &h0
		Sub Binary61Test()
		  ...
		  // Gather up the tests where the decimal flag is 0 (binary arithmetic).
		  Var binaryTests() As Dictionary
		  For Each test As Dictionary In allTests
		    Var initial As New CPUState(test.Value("initial"), test.Lookup("name", ""))
		    If Not initial.DecimalFlag Then
		      binaryTests.Add(test)
		    End If
		  Next test
		  
		  // Run each test.
		  For Each test As Dictionary In binaryTests
		    CPU = New MOS6502.CPU(New MOS6502.Memory)
		    CPU.Reset
